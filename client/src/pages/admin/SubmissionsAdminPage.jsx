...
export default function SubmissionsAdminPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      <h1 className="text-2xl font-bold text-ink-primary mb-6">Modération des soumissions</h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white border border-line rounded-card">
          ...
          <p className="text-ink-secondary">Aucune soumission en attente</p>
        </div>
      ) : (
        <div className="space-y-3">
          {submissions.map(sub => (
            <div key={sub.id} className="bg-white border border-line rounded-card p-4 shadow-sm">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <h3 className="font-semibold text-ink-primary">{sub.title}</h3>
                  <p className="text-sm text-ink-secondary mt-1 line-clamp-2">{sub.description}</p>
                ...
                <div className="flex items-center gap-1 ml-4">
                  <button onClick={() => setSelected(sub)} className="p-2 rounded-lg hover:bg-surface-alt text-ink-light" title="Détails">
                    <Eye size={16} />
                  </button>
      ...
      <Modal isOpen={!!showReject} onClose={() => setShowReject(null)} title="Rejeter la soumission">
        <div className="space-y-3">
          <label className="block text-sm font-medium text-ink-primary">Raison du rejet (optionnel)</label>
          <textarea value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} rows={3} className="w-full border border-line rounded-lg px-3 py-2.5 text-sm resize-y" placeholder="Expliquer la raison..." />
